...
$records_per_page =25;
	  ...
	  (!isset($_GET['start']))? $start = 0 : $start = $_GET['start'];
		...
		$select_count = mysql_query("select count(*) from mid_reg")or die("could not select all".mysql_error());
		$rows = mysql_fetch_row($select_count);
		$total_records = $rows[0];
		
		if(($total_records>0) && ($start<$total_records)){
		$select_pin = mysql_query("select * from mid_reg limit $start, $records_per_page")or die ("could not select from reg table".mysql_error());
