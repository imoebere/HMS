...
if(isset($_POST['sub']))
{
	$username=$_POST['username'];
		...
		$select= "SELECT * FROM mid_reg WHERE username='$username' AND password='$password'";
	$query=mysql_query($select);
	$num = mysql_num_rows($query);
	if ($num > 0){
		$insert= mysql_query("insert into mid_wife values(' ','$username','$password',now())")or 
        ...
        echo "<script >alert('Incorrect E-mail or Password')</script>" ;
		 echo "<script >location('midwife.php')</script>" ;
	}
