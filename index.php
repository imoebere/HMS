...
if(isset($_POST['sub']))
{
	$username=$_POST['username'];
		...
		$select= "SELECT * FROM rec_reg WHERE username='$username' AND password='$password'";
	$query=mysql_query($select);
	$num = mysql_num_rows($query);
	if ($num > 0){
		$insert= mysql_query("insert into reception values(' ','$username','$password',now())")or 
...
<h1><center>  ASABA GENERAL HOSPITAL <br/>ANTE-NATAL RECORD SYSTEM </center></h1>
 <div id="body">
	...
	<div id="footer"> all right reserved (c) 2018 <br/></div>
</div>
</body>
</html>